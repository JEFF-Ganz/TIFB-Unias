...
function renderHeroSlidesTable(slides) {
  ...
  tbody.innerHTML = slides.map((src, index) => `
    <tr>
...
function populateAbout(d) {
  ...
  if (d.photo) {
    ...
    document.getElementById('aboutPreviewSize').textContent = 'Tersimpan di database';
...
async function saveAbout() {
  ...
  let photoUrl = getVal('aboutPhotoUrl').trim() || document.getElementById('aboutPhotoSaved').value;
...
// NAV & FOOTER
// ─────────────────────────────────────────────
function populateNav(d)    { setVal('navLogoText', d.logoText || 'TIF-B CyberSquad'); }
function populateFooter(d) {
  setVal('footerCopyright', d.copyright    || '© 2024 Kelas TIF-B CyberSquad. Hak Cipta Dilindungi.');
  ...
  if (d.developerPhoto && (d.developerPhoto.startsWith('http') || d.developerPhoto.startsWith('/'))) {
...
async function saveNav()    { await save('siteContent/nav',    { logoText: getVal('navLogoText') }, 'Navigasi'); }
async function saveFooter() {
  ...
  if (fileInput.files && fileInput.files[0]) {
    const btn = document.querySelector('#panel-nav .section-card:nth-child(2) .btn-primary');
    btn.disabled = true;
...
function renderPengurusTable(pengurus) {
  ...
  tbody.innerHTML = pengurus.map(([id, p]) => `
    <tr>
      <td>
        ${p.photo 
...
function openAddPengurus() {
  ...
  ['pengurusName','pengurusRole','pengurusQuote','pengurusPhotoUrl'].forEach(id => setVal(id,''));
  clearPhoto('pengurus');
...
async function savePengurus() {
  ...
  if (!name || !role) return showToast('Nama dan Jabatan wajib diisi!', 'error');
  ...
  if (fileInput.files && fileInput.files[0]) {
    ...
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Mengupload...';
    try {
      photoUrl = await uploadToStorage(fileInput.files[0], 'pengurus', 'pengurus');
    } catch(e) {
      showToast('Upload foto gagal: ' + e.message, 'error');
      btn.disabled = false;
      btn.innerHTML = '<i class="fas fa-save"></i> Simpan';
...
function renderMembersTable(members) {
  ...
  tbody.innerHTML = members.map(([id, m]) => `
    <tr>
      <td>
        ${m.photo && m.photo !== '.jpg'
...
function openAddMember() {
  ...
  ['memberName','memberNim','memberQuote','memberPhotoUrl'].forEach(id => setVal(id,''));
  clearPhoto('member');
...
async function saveMember() {
  ...
  if (!name || !nim) { showToast('Nama dan NIM wajib diisi', 'error'); return; }
  ...
  try {
    ...
    showToast(id ? 'Data anggota diperbarui!' : 'Anggota berhasil ditambahkan!');
...
function renderScheduleTable(entries) {
  ...
  tbody.innerHTML = filtered.map(([id, s]) => `
    <tr>
...
function openAddSchedule() {
  ...
  ['scheduleCourse','scheduleLecturer','scheduleRoom','scheduleStart','scheduleEnd'].forEach(id => setVal(id,''));
...
async function saveSchedule() {
  ...
  if (!data.course) { showToast('Nama mata kuliah wajib diisi', 'error'); return; }
  try {
...
function renderGalleryTable(entries) {
  ...
  const catLabels = { all:'Semua', class:'Foto Kelas', event:'Excellent', study:'Studi', social:'Sosial' };
  tbody.innerHTML = entries.map(([id, g]) => `
    <tr>
      <td>
...
function openAddGallery() {
  ...
  ['galleryTitle','galleryDate','galleryPhotoUrl'].forEach(id => setVal(id,''));
...
async function editGallery(id) {
  ...
  if (g.photo && (g.photo.startsWith('http') || g.photo.startsWith('/'))) {
    ...
    document.getElementById('galleryPreviewSize').textContent = 'Disimpan di Firebase Storage';
...
async function saveGalleryItem() {
  ...
  if (!title) { showToast('Judul foto wajib diisi', 'error'); return; }
  ...
  if (fileInput.files && fileInput.files[0]) {
    ...
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Mengupload...';
    try {
      photoUrl = await uploadToStorage(fileInput.files[0], 'gallery', 'gallery');
    } catch(e) {
      showToast('Upload gagal: ' + e.message, 'error');
      btn.disabled = false;
      btn.innerHTML = '<i class="fas fa-save"></i> Simpan';
  ...
  if (!photoUrl) { showToast('Pilih foto atau masukkan URL/nama file', 'error'); return; }

  const data = { title, date: getVal('galleryDate'), photo: photoUrl, category: getVal('galleryCategory') };
  try {
...
function handleFileSelect(prefix, input) {
  ...
  if (prefix === 'music') {
    ...
    document.getElementById('musicPreviewSize').textContent = formatBytes(file.size);
  ...
  reader.onload = (e) => {
    ...
    document.getElementById(`${prefix}PreviewSize`).textContent = formatBytes(file.size);
    ...
    task.on('state_changed',
      ...
      async () => {
        try {
          ...
          setTimeout(() => { if (progContainer) progContainer.classList.remove('show'); }, 1500);
          resolve(url);
        } catch (err) {
          if (progContainer) progContainer.classList.remove('show');
          reject(err);
...
['member','gallery','pengurus','music','heroSlide','about','footer'].forEach(prefix => {
...
function renderMusicTable(musicArray) {
  ...
  tbody.innerHTML = musicArray.map((m, index) => `
    <tr>
...
function openAddMusic() {
  ...
  ['musicTitle','musicArtist','musicAudioUrl'].forEach(id => setVal(id,''));
  clearPhoto('music');
...
async function saveMusic() {
  ...
  if (!title) { showToast('Judul lagu wajib diisi', 'error'); return; }
  ...
  if (!audioUrl) { showToast('Pilih file audio atau masukkan URL', 'error'); return; }
